...
class TestBinanceCard:
    ...
    def test_a_quotation_too_small_to_invert_is_malformed(self) -> None:
        # 1 / 1e-32 to 8 decimals needs 41 digits, one more than the 40 the inversion keeps.
        with pytest.raises(MalformedResponseError, match="cannot be inverted"):
            self.source.parse(self.with_quotation("0." + "0" * 31 + "1"), FETCHED_AT)
        # Just above it the inverse fits: 99999990000000999999900000009999.99900000.
        just_above = self.with_quotation("0." + "0" * 31 + "10000001")
        assert self.source.parse(just_above, FETCHED_AT).value == Decimal(
            "99999990000000999999900000009999.999"
        )
